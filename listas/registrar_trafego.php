    ...
    $updateSQL = sprintf("UPDATE carga1 SET aluno=%s, qtd=%s, hora=%s WHERE id=%s",
    ...
    mysql_select_db($database_conexao, $conexao);
    $query_carg = sprintf("SELECT * FROM carga1 WHERE id = %s", GetSQLValueString($colname_carg, "int"));
...
<link rel="stylesheet" href="../css/paginacao.css" type="text/css" />
    ...
    ?>
    	...
    	<?php 
			...
			$op="Registrou carga horária (trafego) !";
